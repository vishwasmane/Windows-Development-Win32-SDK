...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR: // If we use WM_KEYDOWN, we will have to use hex values of characters.
		...
		case 't': // change content or text
			...
			PAINTSTRUCT ps;
			RECT rcPaintView;
			GetClientRect(ghwndPaintView, &rcPaintView);
			//hdc = GetDC(ghwndPaintView);
			hdc = BeginPaint(ghwndPaintView, &ps);
			SetBkColor(hdc, RGB(0, 255, 0));
			FillRect(hdc, &rcPaintView, CreateSolidBrush(RGB(0, 255, 0)));
			EndPaint(ghwndPaintView, &ps);
			//ReleaseDC(ghwndPaintView, hdc);
			//InvalidateRect(ghwndPaintView, &rcPaintView, TRUE);
			break;
		...
		case 'k': // kill application
			SendMessage(ghwndPaintView, WM_CLOSE, 0, 0);
			break;
...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
	...
	if(strncmp(className, "MSPaintApp", 10) == 0)
	{
		ghwnd = hwnd;
		MessageBox(NULL, TEXT("Paint hack successfull!!!!"), TEXT("Paint window is hacked now"), MB_OK);
		fclose(fp);
		fp = NULL;
		// stop EnumWindowProc
		return FALSE;
...
BOOL CALLBACK EnumChildWindowsProc(HWND hwnd, LPARAM lParam)
{
	...
	// get notepad window handle
	if(strncmp(className, "UIRibbonWorkPane", 4) == 0)
	{
		ghwndPaintView = hwnd;
		MessageBox(NULL, TEXT("Paint edit child window hacked!!!!"), TEXT("Paint edit child window hacked"), MB_OK);
		fclose(fp);
		fp = NULL;
		// stop EnumWindowProc
		return FALSE;
